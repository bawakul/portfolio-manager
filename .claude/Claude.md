...
To distinguish between AI-generated content and human-written notes, use this **hybrid tagging convention** within Notes.md files:

**Source Tags:**
- `#claude` - Content generated by Claude Code
- `#user` - Content written by the user

**Type Tags:**
- `#session` - Work session summaries
- `#reflection` - Insights, learnings, retrospectives
- `#planning` - Roadmaps, task lists, strategic planning
- `#decision` - Key architectural or product decisions

**Usage:**
- Combine source + type tags for clarity (e.g., `#claude #session` or `#user #reflection`)
- Add tags to section headings in Running Notes
- Example: `### [[04 Nov 2025]] - Planning Session #user #planning`
- Notes.md files themselves are **not** tagged as claude-generated (user fills them in)

**When Claude Generates Content:**
- Session summaries → `#claude #session`
- Review outputs → `#claude #reflection`
- Status reports → `#claude #session`

This system enables filtering Obsidian views by authorship and content type.
